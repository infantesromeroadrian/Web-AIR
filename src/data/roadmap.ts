    ...
    phases: [
      {
        ...
        topics: [
          ...
          { name: "P1: RT Tool", st: "pend", h: 100, detail: "Scanner agentic + MCP. Producto MAIN.", res: "garak architecture + PyRIT orchestrator + own design", tags: [{ t: "v5", l: "MAIN" }], items: ["CLI con plugins modulares (Click + entry_points)", "5+ tecnicas de prompt manipulation implementadas", "Scanner para MCP servers: enumerate tools + test boundaries", "Probar 5+ MCP servers del registry oficial", "Pipeline de adversarial prompt discovery automatizado", "Eval framework: report con metricas before/after defense", "80% coverage pytest + mypy strict", "CI/CD con GitHub Actions + release semver", "GitHub repo publico + README ejecutable + demos", "Documentacion EN + 1 blog post de architecture"] },
      ...
      },
      {
        ...
        name: "Visibilidad",
        topics: [
          { name: "Presencia", st: "pend", h: 30, detail: "Posts cortos virales. Distinto de F5 essays.", items: ["6 LinkedIn posts EN sobre hallazgos cortos AI sec", "X / Twitter: 1 thread tecnico por semana", "1 charla meetup local (DEF CON Madrid / OWASP Spain)", "1 charla en conferencia tecnica (BSides / RootedCON)", "LinkedIn optimizado: SEO + featured + skills validated", "GitHub README profile con metrics + projects pinned"] },
          { name: "Network", st: "pend", h: 30, detail: "Communities + interaccion directa con engineers Tier 1.", res: "AI Village DEF CON + MLSecOps + OWASP + AISafety.com", tags: [{ t: "v5", l: "new" }], items: ["Membresia activa AI Village (Discord + DEF CON)", "MLSecOps Community: contribuir 1 articulo o talk", "OWASP LLM Top 10: feedback en RFC abierto", "Replies tecnicos a engineers de Anthropic / OpenAI / NVIDIA en X", "GitHub: comments tecnicos en issues de repos tier-1", "Cold emails 1/semana a researchers (no recruiters)", "2+ conferencias presenciales / ano (BSides / DEF CON / NeurIPS workshop)"] },
          { name: "OSS Maintainership", st: "pend", h: 40, detail: "P1 RT Tool con tracción real.", res: "github.com/<adrian>/<P1>", tags: [{ t: "v5", l: "MAINT" }], items: ["P1 RT Tool: >=100 stars en GitHub", "Aceptar 5+ external PRs + tener 3+ contributors externos", "Roadmap publico + milestones + good-first-issue labels", "Release semver cada 4-6 semanas con changelog", "Documentation site (mkdocs / docusaurus)", "1 mencion en HackerNews / Reddit r/MachineLearning", "1 cita en paper o industry blog"] },
          ...
          { name: "Original Finding", st: "pend", h: 50, detail: "1 hallazgo verificable con traccion publica.", res: "arXiv + own blog + LessWrong + AlignmentForum", tags: [{ t: "v5", l: "crit" }], items: ["Identificar gap original (no replica de paper existente)", "Implementacion + ablations rigurosas", "Writeup tecnico EN >=3000 palabras", "Publicar en blog propio + cross-post LessWrong", "Tweet thread + LinkedIn post el dia del release", "Trackear traccion: >=1 cita en paper / >=100 stars / >=1 industry mention", "Defensa publica en 1 podcast o charla tecnica", "Si encaja: convertir en submission a arXiv (sinergia con F5 boss)"], boss: true },
        ],
      },
    ],
  },
